...
When AI agents fetch individual documentation files (via API, web scraping, or direct download), they receive files without repository context. A link like `[README.md](../../README.md) ("The Invisible Users" at <https://github.com/Digital-Domain-Technologies-Ltd/MX-hub/blob/main/README.md>)` becomes meaningless - what's two directories up from a standalone file? Where does the path resolve to? Which repository? What branch? The link destination cannot be determined without the original file system structure.
